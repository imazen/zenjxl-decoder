...
fn decode_cms_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group("decode_cms");
    group.sampling_mode(SamplingMode::Flat);
        ...
        group.bench_with_input(
            ...
            |b, bytes| {
                b.iter(|| {
                    ...
                    opts.cms = Some(Box::new(Lcms2Cms));
                    decode_frames(
                        &mut input,
                        opts,
                        None,
...
criterion_group!(
    ...
    targets = decode_benches, decode_cms_benches
);
