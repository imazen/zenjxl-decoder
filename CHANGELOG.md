...
### Added
- **Level 5 feature bounds, on by default.** New `JxlDecoderLimits::max_codestream_level` (`JxlCodestreamLevel::Level5` or `Level10`, builder `with_max_codestream_level`) bounds total spline area, total patch area and the number of modular channels after transforms; violations fail with `Error::LimitExceeded` (`SplinesAreaTooLarge` for splines, as before). Default and `restrictive()`: Level 5. `unlimited()`: Level 10. Files that need Level 10 now need `with_max_codestream_level(JxlCodestreamLevel::Level10)`; before, splines were held to Level 10 bounds and patches and channel counts were unbounded. Palette meta-channels are also capped, summed over a transform list, at `max_pixels` samples (group-local palettes at 2^26). All fixtures and corpus images decode under the new default. Ported from upstream jxl-rs `7dad7ef`, `9e7caa4` and `accec18`, which use three `force_level5_*` booleans instead of one level. Tests `channel_count_bounded_by_level`, `squeeze_growth_bounded_by_level`, `palette_samples_bounded`, `level_area_bounds`, and a Level 5 rejection in the spline draw-cache test. The CLI gains `--allow-level10`, and its `--allow-partial-files` now also sets `recover_partial_image`.
...
### Changed
- **`WeightedPredictorState`'s per-position error storage is `Vec<[u32; NUM_PREDICTORS]>` instead of a flat `Vec<u32>`.** `get_errors_at_pos`/`get_errors_at_pos_mut` indexed a slice and converted it with `first_chunk()`/`first_chunk_mut().unwrap()` on every call in the weighted-predictor hot loop; they now index the array `Vec` directly, with no slicing or `unwrap()`. Same values, same position-major layout, no behavior change (`fa5689a`). `decode_bench`'s `modular/*` group showed no measurable difference on the two largest fixtures and a noisy 10-45% improvement on the smaller/faster ones (single run per side, not zenbench's interleaved A/B, so treat as directional rather than a precise figure).
...
- **Overlapping VarDCT blocks were accepted.** A tall block placed on an earlier row reaches into later rows; a wide block placed there afterwards overwrote its transform-map cells instead of being rejected as libjxl does. Placement now checks the block's footprint first (`InvalidVarDCTTransformMap`). Lehmer-code decoding also rejects permutations longer than 2^30, where `next_power_of_two` on `u32` overflows. Ported from upstream jxl-rs `895d743` and `0ed1c44`; neither has a dedicated test (the first needs a crafted modular HF-metadata stream, the second a 2^30-entry slice).
- **Truncated metadata boxes allocated their declared size.** `jhgm`, `jxli`, `Exif`, `xml ` and `brob` buffers were reserved and zero-filled to the box's declared length on the first read (up to 256 MB for `jhgm`, 16 MB for the others), so a 136-byte file claiming a 200 MB gain map committed 200 MB. Buffers now grow geometrically with the data actually delivered. Test `truncated_large_metadata_box_does_not_allocate_declared_size` (old code: 209,715,200 bytes of capacity for 100 buffered bytes). Found while checking upstream jxl-rs `11587ae`/`f7725bc`, whose short-read truncation and seek-offset fixes this fork already had.
- The per-frame `max_pixels` check now bounds the frame's upsampled size, which is what gets rendered (up to 64x the coded area with 8x upsampling), not its coded size. Ported from upstream jxl-rs `c07c7d5`.
- Preview headers wider or taller than 4096 pixels are rejected (the header coder admits heights up to 5440, and the aspect ratio can double the width); the incremental TOC reader reserves at most 4096 entries up front and grows as entries arrive, instead of reserving the frame header's untrusted count before any TOC bytes are read. Ported from upstream jxl-rs `d13a505`; this fork's image-size check (at most 2^30 pixels) already covered its main-image half. Test `preview_dimensions_capped_at_4096`.
- The 2x/4x/8x upsampling kernels no longer pre-broadcast their weights into a `[[F32Vec; 25]; N * N]` stack array (about 100 KB for 8x with AVX-512, a risk on small thread or WASM stacks); weights are broadcast at use, with the same FMA order and bit-identical output. The kernel table is built directly on the heap. Ported from upstream jxl-rs `3e46b76`; speed not measured.
- **Patch row counting was quadratic, and the spline control-point total could wrap.** The per-row patch count looped over every row of every patch (O(patches × rows)); it now uses a difference array. The running spline control-point total used unchecked `u32` addition, so a spline claiming ~`u32::MAX` points wrapped it under the limit in release builds. Ported from upstream jxl-rs `9e7caa4`, without its default Level 5 patch-area limit, which adds a public option and is pending review.
- **`file_length()` was wrong after skipping the last frame.** The frame-skip path left the skipped section bytes counted as buffered, so `record_file_length` subtracted them again: skipping every frame of `green_queen_vardct_e3.jxl` reported 31 bytes instead of 88,995. The skip path also sums TOC section lengths in `u64`, which could overflow `usize` on 32-bit targets (upstream jxl-rs `d71c785`). Test `file_length_after_skipping_last_frame`; this part is specific to this fork.
- **Restoration-filter and render-pipeline validation from upstream jxl-rs `d71c785` and `ba34f3f`.** Gaborish weights with `1 + 4·(w1 + w2) ≈ 0` and non-positive or non-finite modular EPF sigmas are rejected at the frame header; both were divisors that turned every filtered pixel into inf/NaN. The render-pipeline check that all channels written by one Save stage share a downsampling never recorded its first value, so it could not fire; it now works and ignores channels the stage does not write. The patch row-count table is reserved fallibly, and the simple pipeline sizes its input rows by the input type. Tests `restoration_filter_rejects_degenerate_weights` and `save_stage_rejects_mixed_downsampling`.
- **Negative base colour correlations were not range-checked.** `ColorCorrelationParams::read` rejected only `base_correlation_x/b > 4`, so any negative value passed; it now checks the magnitude, as libjxl does. Modular channel decoding also stops at the channel that overran its section instead of decoding the rest from padding (the section's final-state check already rejected such streams). Ported from upstream jxl-rs `b0ae00b`; test `base_correlation_limit_applies_to_magnitude`. The commit's debug assertions and `lf_preview` unwrap cleanup are not ported: the fork's preview code already checks the format first.
- **Colour encodings that cannot produce an ICC profile are rejected at the header.** A custom white point or custom primaries whose chromatic adaptation or RGB-to-XYZ matrix is degenerate or outside ICC s15Fixed16 now fail `ColorEncoding::check`; before, `check` accepted them. The ICC tag list also no longer truncates 64-bit tag offsets and sizes to `u32` (an offset of 2^32 + 128 wrapped to 128 and passed the bounds check) or overflows computing the first tag offset from a huge tag count. Ported from upstream jxl-rs `0f41860` and `96b0be3`, using the latter's resolved white point: `0f41860` alone would validate custom primaries against an unset white. Tests `test_degenerate_primaries_cannot_create_icc`, `tag_start_above_u32_is_rejected` and `tag_count_overflow_is_rejected`.
- The incremental ICC reader now validates the stream preamble (commands size within the stream, output size bounded and not dwarfed by the stream) as soon as its first 20 bytes are decoded, instead of only after the whole stream (up to 16 MB). This fork's existing output/input amplification cap already bounded the degenerate-histogram case; this rejects impossible preambles earlier and with upstream's errors. Ported from upstream jxl-rs `a72cbc0`.
- ICC profile synthesis serialized the profile twice (a duplicated block, same result, wasted work); dead HLG `encoded_from_display`/`oetf` removed; `BufReader::skip` beyond `i64::MAX` now seeks to the end instead of issuing a clamped relative seek that fails with EINVAL on file descriptors. Ported from upstream jxl-rs `c1e2e3d`; its box-parser underflow fix was already covered here by saturating file-length accounting.
- **Incremental decoding could stall at the frame header on valid files.** The TOC reader padded its `OutOfBounds` count by 2 bytes per remaining TOC entry, so the refill loop waited for bytes the TOC did not need; `truncated_squeeze_missing_avg.jxl` asked for 167 more bytes and never reached the frame body, where upstream proceeds. It now reports the exact shortfall (upstream jxl-rs `00c67ce`), together with upstream `9763730`, which defers decoding a permuted TOC's permutation until a lower bound for the whole TOC is buffered. That second change is required: a first landing without it (`fd40f84c`, reverted in `736506c8`) broke 1-byte-chunk decoding of the conformance progressive images with `AlphabetTooLargeHuff`. The incremental TOC reader also no longer records an entry or permutation before checking for a read overrun. `flush_truncated_squeeze_missing_avg` now requires a flush and fails with the old estimate. This corrects an earlier note in `docs/UPSTREAM_SYNC.md` that had attributed the stall to this fork's flush design.
- **Squeezing an empty channel could divide by zero.** `check_squeeze_params` now rejects a squeeze over a zero-width or zero-height channel (`TooManySqueezes`), and `get_grid_rect` returns the empty rect before computing `bx % lf_grid_dim`, which divided by zero for a zero-size grid. Ported from upstream jxl-rs `f809cce` (its `avg_rect` hunk has no counterpart in this fork's restructured squeeze application). Regression test `squeeze_of_empty_channel_is_rejected`.
- **Entropy-header validation gaps.** The simple Huffman table's duplicate-symbol check used `symbols[..i]` instead of `[..=i]`, so adjacent duplicates like `[0, 0]` were accepted; the fixed-width context-map form skipped the hole check the entropy-coded form runs (`[0, 2]` was accepted); and the LZ77 special-distance multiply could overflow unchecked. Ported from upstream jxl-rs `838d865`; tests `test_simple_table_duplicate_symbols` and `simple_context_map_rejects_holes` fail on the old code.
- **Skipping input could desynchronise the stream on short reads.** The default `JxlBitstreamInput::skip` ignored how many bytes `read` returned and counted every chunk as fully skipped; with a reader delivering 3 bytes per call, a reported 10-byte skip consumed 3. `BufReader::skip` cast the count with `as i64`, so `usize::MAX` became a one-byte seek backwards. Ported from upstream jxl-rs `f809996`; tests `default_skip_counts_short_reads` and `bufreader_skip_huge_does_not_seek_backwards` fail on the old code.
- `fast_pow2f` and the xorshift seed use wrapping arithmetic (debug builds panicked on overflow; release results are unchanged), and `ceil_log2`/`mirror` assert their non-zero preconditions in debug builds. Ported from upstream jxl-rs `29bf689`.
- **Hostile spline coefficients could bypass the spline area limit, or panic.** `QuantizedSpline::dequantize` used `i32::abs` (overflows on `i32::MIN`) and unsaturated `u64` products; measured on the old code, extreme coefficients wrapped the area estimate to 8.6e9 — below the 2.2e12 limit, so the oversized spline was accepted — and debug builds panicked with "attempt to negate with overflow". Now `unsigned_abs` and saturating arithmetic. `ShiftRightCeil::shrc` also overflowed near `MAX` (`u32::MAX.shrc(1)` wrapped to 0). Ported from upstream jxl-rs `ee7c7c5`; regression tests `dequantize_saturates_area_estimate_on_extreme_coefficients` and extended `test_shrc`.
- **Partial and parallel renders could treat a neighbour's pixels as ready.** `ready_image_area` computed each strip's boundaries independently, so in a group narrower than two borders the leading strip reached past the group into the next one — which may not be decoded yet — and overlapped the trailing strip. The boundaries now come from one monotonic `boundary_points` helper. Ported from upstream jxl-rs `964211a`; regression test `ready_image_area_strips_do_not_overlap_in_narrow_group` (fails on the old math: strips overlapped on columns 22..26).
- **Progressive modular frames could decode shift-0 channels twice.** `Passes::downsampling_bracket` gave two passes overlapping shift ranges when a `downsample` entry of 1 ended an earlier pass, so a later pass re-read channels already finished and misparsed the stream. The brackets now partition `[0, 3)` without overlap, empty passes get `(1, 0)`. Ported from upstream jxl-rs `8d71a02`; regression tests `downsampling_bracket_does_not_reuse_shift_zero` and `downsampling_bracket_partitions_common_layouts` (the old code fails both).
