...
### Fixed
- **Incremental decoding could stall at the frame header on valid files.** The TOC reader padded its `OutOfBounds` count by 2 bytes per remaining TOC entry, so the refill loop waited for bytes the TOC did not need; `truncated_squeeze_missing_avg.jxl` asked for 167 more bytes and never reached the frame body, where upstream proceeds. It now reports the exact shortfall. Ported from upstream jxl-rs `00c67ce`; `flush_truncated_squeeze_missing_avg` now requires a flush and fails with the old estimate. This also corrects an earlier note in `docs/UPSTREAM_SYNC.md` that had attributed the stall to this fork's flush design.
