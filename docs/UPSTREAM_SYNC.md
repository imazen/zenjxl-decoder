...
| e7405e0 #752 (+3c4f224 #777) | out-of-order `jxlp` boxes (ftyp minor version 1; `cjxl --output_mode 2`) | **PORTED** (`29c6dd4`, batch 3; bounded buffering, `container_boxes` + `jxlrs-752` tests). **Followed up 2026-09-08:** the port had five defects, all fixed here (`0ca74c1`, `ed1fa3d`, `c977405`, `fe2ab2e`) — see CHANGELOG. One of them exists upstream too and we sent the fix back as **libjxl/jxl-rs#956** (empty `jxlp` box read as EOF). The other four are fork-only: our `MAX_BUFFERED_BOXES = 1024` cap (upstream has none; libjxl's is `1 << 20`), the `BufferingOooJxlp` zero-length read, the header refill at input EOF, and sections spanning a `jxlp` box boundary — that last one also broke **in-order** containers with small boxes, which upstream and djxl decode. When #956 lands, our `injected_jxlp` analogue (`try_inject_next_buffered_jxlp`) already carries the same behaviour; do not re-port it. |
...
| 035477c #678, 2cddd90 #702 | animation scan/seek API | **PORTED** (issue #11): same public surface as current upstream (`scanned_frames`, `VisibleFrameInfo`, `VisibleFrameSeekTarget`, `start_new_frame` with internal skipping, `scan_frames_only`); the seek internals are the fork's own (per-box `CodestreamBoxRecord` position mapping, dependency slots keyed by file offset, fresh `DecoderState` with restored frame counters) since the fork keeps its pre-#828 parser. Deviations: `file_offset` / `seek_target` are `Option`s (`None` for out-of-order `jxlp` files). |
