...
pub fn do_hsqueeze_step(
    ...
    let (w, _h) = in_res.size();
    // Another shortcut: when output row has just 1px
    if w == 0 {
        for y in 0..out.data.size().1 {
            out.data.row_mut(y)[0] = in_avg.row(y)[0];
        }
        return;
    }
    // Otherwise: 2 or more in row
    hsqueeze_scalar(in_avg, in_res, in_next_avg, out_prev, &mut out.data);
}
...
pub fn do_vsqueeze_step(
    ...
    vsqueeze_scalar(in_avg, in_res, in_next_avg, out_prev, out);
}
