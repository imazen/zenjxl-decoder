            ...
            while !toc_parser.is_complete() {
                match toc_parser.read_step(&mut br) {
                    ...
                    Err(e) if matches!(e.error(), Error::OutOfBounds(_)) => {
                        ...
                        self.non_section_buf.consume(bits / 8);
                        self.non_section_bit_offset = (bits % 8) as u8;
                        // Report exactly what this step is missing. Padding it
                        // by 2 bytes per remaining TOC entry made the refill loop
                        // wait for bytes the TOC does not need, stalling valid
                        // files at the frame header. Upstream jxl-rs 00c67ce.
