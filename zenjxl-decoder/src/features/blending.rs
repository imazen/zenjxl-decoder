...
///
/// Vectorised, allocation-free port of upstream jxl-rs #709 / #818 / #821:
/// the compositing modes run as `simd_function!` kernels over the fork's SIMD
/// layer, results are written straight into `bg`, and only the extra
/// channels' previous values are snapshotted into `tmp` (needed because the
/// alpha reads must see pre-blend alpha). Operand order matches the scalar
/// reference exactly, so output is bit-identical to it.
