    ...
    pub fn to_f32(self) -> f32 {
        ...
        let f32_bits = if exp == 0 {
            if mant == 0 {
                ...
                // f16 subnormals are (mant/1024) * 2^-14: the effective exponent
                // is -14, not -15 (the normal-range bias). The leading-1 search
                // above shifted by `e`, so rebias from -14-e into f32.
