...
impl<const N: usize, const SHIFT: u8> Upsample<N, SHIFT> {
    pub fn new(ups_factors: &CustomTransformData, channel: usize) -> Self {
        ...
        // Flattened kernels, row-major: kernel[oy][ox] -> flat_kernels[oy * N + ox],
        // each 5x5 tap block row-major. Built directly on the heap; the old
        // intermediate `[[[[f32; 5]; 5]; N]; N]` lived on the stack.
        ...
        for i in 0..5 * n {
            for j in 0..5 * n {
                ...
                flat_kernels[(m - j / 5) * N + (i / 5)][(4 - (j % 5)) * 5 + (i % 5)] = w;
                flat_kernels[(j / 5) * N + (m - i / 5)][(j % 5) * 5 + (4 - (i % 5))] = w;
                flat_kernels[(m - j / 5) * N + (m - i / 5)][(4 - (j % 5)) * 5 + (4 - (i % 5))] = w;
            }
        }
...
macro_rules! kernel_conv {
    ($d:expr, $kv:expr, $r0:expr, $r1:expr, $r2:expr, $r3:expr, $r4:expr, $x:expr) => {{
        // Weights are broadcast at use rather than pre-broadcast into a
        // [[F32Vec; 25]; N * N] array (~100 KB of stack for 8x with AVX-512).
        // Upstream jxl-rs 3e46b76.
        let kv: &[f32; 25] = $kv;
        // Compute 5x5 kernel using FMA with 3-way ILP
        // Row 0
        let mut acc0 = <D::F32Vec>::load($d, &$r0[$x..]) * <D::F32Vec>::splat($d, kv[0]);
        let mut acc1 = <D::F32Vec>::load($d, &$r0[$x + 1..]) * <D::F32Vec>::splat($d, kv[1]);
        let mut acc2 = <D::F32Vec>::load($d, &$r0[$x + 2..]) * <D::F32Vec>::splat($d, kv[2]);
        acc0 = <D::F32Vec>::load($d, &$r0[$x + 3..]).mul_add(<D::F32Vec>::splat($d, kv[3]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r0[$x + 4..]).mul_add(<D::F32Vec>::splat($d, kv[4]), acc1);
        // Row 1
        acc2 = <D::F32Vec>::load($d, &$r1[$x..]).mul_add(<D::F32Vec>::splat($d, kv[5]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r1[$x + 1..]).mul_add(<D::F32Vec>::splat($d, kv[6]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r1[$x + 2..]).mul_add(<D::F32Vec>::splat($d, kv[7]), acc1);
        acc2 = <D::F32Vec>::load($d, &$r1[$x + 3..]).mul_add(<D::F32Vec>::splat($d, kv[8]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r1[$x + 4..]).mul_add(<D::F32Vec>::splat($d, kv[9]), acc0);
        // Row 2
        acc1 = <D::F32Vec>::load($d, &$r2[$x..]).mul_add(<D::F32Vec>::splat($d, kv[10]), acc1);
        acc2 = <D::F32Vec>::load($d, &$r2[$x + 1..]).mul_add(<D::F32Vec>::splat($d, kv[11]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r2[$x + 2..]).mul_add(<D::F32Vec>::splat($d, kv[12]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r2[$x + 3..]).mul_add(<D::F32Vec>::splat($d, kv[13]), acc1);
        acc2 = <D::F32Vec>::load($d, &$r2[$x + 4..]).mul_add(<D::F32Vec>::splat($d, kv[14]), acc2);
        // Row 3
        acc0 = <D::F32Vec>::load($d, &$r3[$x..]).mul_add(<D::F32Vec>::splat($d, kv[15]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r3[$x + 1..]).mul_add(<D::F32Vec>::splat($d, kv[16]), acc1);
        acc2 = <D::F32Vec>::load($d, &$r3[$x + 2..]).mul_add(<D::F32Vec>::splat($d, kv[17]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r3[$x + 3..]).mul_add(<D::F32Vec>::splat($d, kv[18]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r3[$x + 4..]).mul_add(<D::F32Vec>::splat($d, kv[19]), acc1);
        // Row 4
        acc2 = <D::F32Vec>::load($d, &$r4[$x..]).mul_add(<D::F32Vec>::splat($d, kv[20]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r4[$x + 1..]).mul_add(<D::F32Vec>::splat($d, kv[21]), acc0);
        acc1 = <D::F32Vec>::load($d, &$r4[$x + 2..]).mul_add(<D::F32Vec>::splat($d, kv[22]), acc1);
        acc2 = <D::F32Vec>::load($d, &$r4[$x + 3..]).mul_add(<D::F32Vec>::splat($d, kv[23]), acc2);
        acc0 = <D::F32Vec>::load($d, &$r4[$x + 4..]).mul_add(<D::F32Vec>::splat($d, kv[24]), acc0);
