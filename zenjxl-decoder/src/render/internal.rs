...
impl<Buffer> RenderPipelineShared<Buffer> {
    ...
    pub fn group_size_for_channel(
        ...
        // Channels that no stage consumes have no type at all. Callers may
        // still ask for a scratch buffer for them -- VarDCT always decodes
        // three colour channels, but a grayscale pipeline only ever reads the
        // first -- and the data written there is discarded, so any type is
        // acceptable. Unwrapping here panicked on such a frame
        // (`tests/testdata/vardct_grayscale_unused_channel.jxl`, 61 bytes).
        if let Some(ty) = ty
